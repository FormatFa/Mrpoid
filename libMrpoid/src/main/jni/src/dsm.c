...
#define 		NO_EXRAM 			0
#define 		DSM_MEM_SIZE		(gEmulatorCfg.memSize*1024*1024)	//DSM内存大小
...
{
	DsmSocketClose();
	FREE_SET_NULL(gEmulatorParams.exMem);
...
int32 mr_getUserInfo1(mr_userinfo* info)
{
	...
	strncpy(info->IMEI, dsmIMEI, 15);
	strncpy(info->IMSI, dsmIMSI, 15);
	strncpy(info->manufactory, "mrpoid", 8);
	strncpy(info->spare, "天使之翼", 12);
	strncpy(info->type, "android ", 8);

	info->ver = MAKE_PLAT_VERSION(gEmulatorCfg.platform, 8, 0, 18, 0);
...
//int32 mr_getUserInfo0(mr_userinfo* info)
//{
//	return gEmulatorCfg.platform == 1? mr_getUserInfo2(info) : mr_getUserInfo1(info);
//}
...
void mr_cacheFlush(int id)
{
	...
	cacheflush((long)gEmulatorParams.vm_mem_base, (long)(gEmulatorParams.vm_mem_base + gEmulatorParams.vm_mem_len), 0);
}
...
int32 mr_mem_get(char** mem_base, uint32* mem_len)
{
	...
	*mem_len = alloc_size;
	...
	gEmulatorParams.vm_mem_len = alloc_size;
...
int32 mr_mem_get(char** mem_base, uint32* mem_len){
	...
	*mem_base = buffer;
	*mem_len = len;
	gEmulatorParams.vm_mem_base = buffer;
...
uint32 mr_getTime(void)
{
	...
	uint32 s = ret==0? ((t.tv_sec - gEmulatorParams.dsmStartTime.tv_sec)*1000 + (t.tv_usec - gEmulatorParams.dsmStartTime.tv_usec)/1000) : 0;
...
void DsmPathInit(void)
{
	...
	GBToUTF8String(buf, buf2, sizeof(buf2));
...
int32 mr_info(const char* filename)
{
	...
	if (ret != 0){
		if(gApiLogSw.showFile) LOGI("  is err");
		return MR_IS_INVALID;
	}

	if (s1.st_mode & S_IFDIR){
		if(gApiLogSw.showFile) LOGI("  is dir");
		return MR_IS_DIR;
	}else if (s1.st_mode & S_IFREG){
		if(gApiLogSw.showFile) LOGI("  is file");
		return MR_IS_FILE;
	}else {
		if(gApiLogSw.showFile) LOGI("  is other");
		return MR_IS_INVALID;
	}
}
...
int32 mr_findGetNext(MR_FILE_HANDLE search_handle, char* buffer, uint32 len)
{
	...
	if((pDt = readdir(pDir)) != NULL){
		...
		//查找完毕
//		LOGE("mr_findGetNext: readdir FAIL!");
	}
...
{
	if(gEmulatorCfg.androidDrawChar) {
...
{
	if(gApiLogSw.showMrPlat) LOGI("mr_plat(code:%d, param:%d)",
		code, param);
...
{
	if(gApiLogSw.showMrPlat) LOGI("mr_platEx(code:%d)", code);

	switch(code)
	{
	case MR_MALLOC_EX: //申请屏幕缓冲区，这里给的值即 VM 的屏幕缓冲区
		{
