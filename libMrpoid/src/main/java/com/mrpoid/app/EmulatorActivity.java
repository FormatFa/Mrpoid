...
//	private SmsReceiver mSmsReceiver;
	...
	private void initEntrys() {
		String path = getIntent().getStringExtra(MrpRunner.INTENT_KEY_ENTRY_MRP);
		String activity = getIntent().getStringExtra(MrpRunner.INTENT_KEY_ENTRY_ACTIVITY);

		SharedPreferences appsp = getSharedPreferences(APP_ACTIVITY_NAME, 0);
		if(path == null) {
			path = appsp.getString(MrpRunner.INTENT_KEY_ENTRY_MRP, null);
		} else {
			//保存路径，下次启动时候丢失也可以拿到
			appsp.edit().putString(MrpRunner.INTENT_KEY_ENTRY_MRP, path).commit();
		}
		
		if(activity == null) {
			activity = appsp.getString(MrpRunner.INTENT_KEY_ENTRY_ACTIVITY, null);
		} else {
			appsp.edit().putString(MrpRunner.INTENT_KEY_ENTRY_ACTIVITY, activity).commit();
		}
		...
		mEntryMrp =new java.io.File( path).getAbsolutePath();
	...
	void initPad() {
		// 虚拟键盘
		keypad = Emulator.getInstance().getKeypad();
