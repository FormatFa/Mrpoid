...
//		LibLoader.load(mContext);
		...
		mKeypad = new Keypad(mContext, false);
	...
	public void setRunMrp(String path) {
		if(path == null) {
			UIUtils.toastMessage(mContext, "启动文件路径错误101！"+path);
			return;
		}
		
		EmuLog.i(TAG, "inputPah=" + path);
		EmuLog.i(TAG,"sdcard root:"+SDCARD_ROOT);
		//1.是不是绝对路径
...
//
		...
		if(file.isFile())
		{
			EmuLog.i(TAG,"Input is a file path:"+file.getAbsolutePath());
			//is copyed？
			File checkIsCopy = getVmFullFilePath(filename);
			if(!checkIsCopy.exists())
			{
				if(checkIsCopy.getParentFile().canWrite()==false)
					EmuLog.e(TAG,"can not write");
				EmuLog.i(TAG,"copy file to:"+checkIsCopy.getAbsolutePath());
				if (FileUtils.FAILED == FileUtils.copyTo(checkIsCopy,file)) {
				//	throw new RuntimeException("file path invalid, copy file to VmWorkPath fail!");
			try {
                InputStream is = new FileInputStream(path);
                byte[] buff = new byte[is.available()];
                is.read(buff);
                OutputStream os = new FileOutputStream(checkIsCopy);
                os.write(buff);
            }
            catch (Exception e)
            {
                EmuLog.e(TAG,"copy exception:"+e.toString());
            }
				}
...
//		log.i("drawImage " + x)
	...
	public void initPath() {
		...
		if(!FileUtils.isSDAvailable(DEF_MIN_SDCARD_SACE_MB)) {
			...
			EmuLog.i(TAG,"SDcared un available");
//			Toast.makeText(context, "没有SD卡！", Toast.LENGTH_SHORT).show();
		...
		sb.append("sd path:"+MrpoidSettings.sdPath);
